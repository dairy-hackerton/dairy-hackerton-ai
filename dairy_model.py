...
class DIARY:
    ...
    def diary_prompt(self, data):
        self.prompt_template = f"""
            ...
            위 내용을 바탕으로 톤에 충실한 이모티콘을 사용하여 톤과 컨디션에 가까운 느낌으로 10문장 이내의 한국어 일기를 작성해주세요.
            날짜를 빼주시고, content=와 같이 일기와 상관없는 문자는 꼭 빼주세요.
            """
    ...
    def summary(self, response_kor):
        ...
        재미있게 요약해주되, content=와 같이 일기와 상관없는 문자는 꼭 빼주고 요약한 문장만 도출해줘.
    ...
    def translate(self, language, response_kor):
        translate_prompt = f"""당신은 {language}와 영어에 원어민 수준의 유창성을 갖춘 전문 번역가입니다.  
                    ...
                    반드시 번역한 내용을 도출해야 하며, content=와 같이 일기와 상관없는 문자는 꼭 빼주고 번역한 내용만 도출하세요.
